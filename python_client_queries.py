...
## Query 4 ##
cursor.execute("select \"domain_ext\", count(*) as requests_per_domain from \"server_logs_test\" group by \"domain_ext\" order by requests_per_domain desc")
...
## QUERY 6 ##
cursor.execute(" select \"country\", count(*) as requests_per_region from \"server_logs_test\" group by \"country\" order by requests_per_region desc")
...
## QUERY 7 ##
cursor.execute("select \"url\", avg(count_per_day) as avg_per_day from (select DAYOFYEAR(TO_TIME(\"server_ts\")) as day_of_year,\"url\", count(\"url\") as count_per_day from \"server_logs_test\" group by DAYOFYEAR(TO_TIME(\"server_ts\")) , \"url\") group by \"url\" order by avg_per_day desc")
...
## QUERY 8 ##
cursor.execute("select count(*) as number_of_image_requests from \"server_logs_test\" where (\"url\" like '%.gif')")
...
## QUERY 9 ##
cursor.execute("select \"month\", \"day\", sum(TO_NUMBER(\"count\")) as requests from \"sl_metadata\" group by \"month\",\"day\" order by \"month\",requests desc")
...
cursor.execute("select WEEK(TO_TIME(\"date\")) as week, sum(TO_NUMBER(\"count\")) as requests from \"sl_metadata\" group by week,\"dayofweek\" order by requests desc")
...
cursor.execute("select \"month\", \"day\", \"hour\", sum(TO_NUMBER(\"count\")) as requests from \"sl_metadata\" group by \"month\",\"day\",\"hour\" order by \"month\",\"day\",requests desc")
...
for ts in High_failure_timeranges.Server_Timestamp:
    #print(str(ts))
    cursor.execute("select \"host\" from \"server_logs_test\" where TO_TIMESTAMP(\"server_ts\") >= TO_TIMESTAMP('{}') and TO_TIMESTAMP(\"server_ts\") <= TO_TIMESTAMP('{}') + (0.00347222222)".format(str(ts),str(ts)))
